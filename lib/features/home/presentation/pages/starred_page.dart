...
class StarredPage extends StatelessWidget {
  ...
  Future<void> _unstarDoc(String id) async {
    await FirebaseFirestore.instance
        .collection('words')
        .doc(id)
        .update({'is_starred': false, 'updatedAt': FieldValue.serverTimestamp()});
  }
                ...
                children: [
                  if (pro.isNotEmpty)
                    Padding(
                      ...
                      child: Text(pro, style: const TextStyle(color: Colors.black54)),
                    ),
                  if (mean.isNotEmpty)
                    Padding(
                      padding: const EdgeInsets.only(bottom: 8),
                  ...
                  if (ex.isNotEmpty)
                    Text(ex, style: const TextStyle(fontSize: 14, color: Colors.black54)),
                ],
              ),
            ),
          ),
    ...
    return Scaffold(
      ...
      body: uid == null
        ...
        builder: (context, snap) {
          ...
          if (docs.isEmpty) {
            return const Center(child: Text('Chưa có từ nào được đánh dấu'));
          }
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              final subtitle = [
                if ((m['pronunciation'] ?? '').toString().isNotEmpty) m['pronunciation'],
                if ((m['meaning'] ?? '').toString().isNotEmpty) m['meaning'],
              ].join(' • ');

              return Card(
                clipBehavior: Clip.antiAlias, // Cắt nội dung theo border
                ...
                child: ListTile(
                  ...
                  onTap: () => _openDetailDialog(context, m: m, id: d.id),
                ),
              );
            },
          );
